      ...
      <v-card-text>
        <v-form @submit.prevent="submitForm">
          <v-row>
            ...
            <v-col cols="12" sm="6">
              <v-switch v-model="material.avalibilityStatus" label="Estado de Disponibilidad"></v-switch>
            </v-col>
            <v-col cols="12">
              <v-menu v-model="menu" :close-on-content-click="false" transition="scale-transition" offset-y min-width="auto">
                <template v-slot:activator="{ on }">
                  <v-text-field v-model="formattedLastMaintenance" label="Último Mantenimiento" readonly v-on="on" required></v-text-field>
                </template>
...
export default {
  ...
  data() {
    return {
      material: new Material(null, '', false, new Date().toISOString().substr(0, 10)),
      menu: false,
      loading: false,
      success: false
    };
  ...
  methods: {
    ...
    validateForm() {
      ...
      if (!this.material.materialName) {
        // Manejar el error de campo obligatorio
        return false;
      }
      return true;
    },
