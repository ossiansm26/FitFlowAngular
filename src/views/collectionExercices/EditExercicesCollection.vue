<template>
    <v-container>
      ...
      <v-row justify="center">
        <v-col cols="12" sm="8" md="6">
          <v-card>
            <v-card-title class="headline center">Edit Collection Exercises</v-card-title>
            <v-card-text>
  ...
  export default {
    ...
    methods: {
      saveCollection() {
        axios.put(`http://localhost:3001/api/collectionExercices/update/${this.collection.id}`, this.collection)
          .then(() => {
      ...
      fetchCollection(collectionId) {
        axios.get(`http://localhost:3001/api/collectionExercices/getById/${collectionId}`)
          .then(response => {
